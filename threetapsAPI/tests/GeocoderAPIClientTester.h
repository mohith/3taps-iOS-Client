...
// ################################################################################################
//
// GeocoderAPIClientTester
//
//     This class implements a series of unit tests for the |GeocoderAPIClient| class.
//
//     Note that, because the |GeocoderAPIClient| runs asyncronously, it is tricky to implement
...
@interface GeocoderAPIClientTester : NSObject <GeocoderAPIClientDelegate> {
  @private
  GeocoderAPIClient* _client;   // The |GeocoderAPIClient| we are testing.
...
@property (nonatomic, retain) GeocoderAPIClient* _client;
...
//
//     Run the unit tests on our |GeocoderAPIClient|.
//
//     We start the first API test, which runs asyncronously.  Once the first test has finished,
//     the second test is started, and so on until all the Geocoder API tests have been completed.
//     Once the final test has been finished, the given method of the given object will be called
//     to notify the testing system that the Geocoder API tests have finished.
